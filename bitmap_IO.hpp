...
template <
    ...
    //typename cmap_t=radiation_cmap_t>
void dump_bitmap(
    value_t *data,
    index_t height,
    index_t width,
    std::string filename, int scale=1) {
...
int height2=-height; // for top-down-format
    info_header[ 8] = height2 >>  0;
    info_header[ 9] = height2 >>  8;
    info_header[10] = height2 >> 16;
    info_header[11] = height2 >> 24;
...
// use 0, 255 if min,max are in that range
if (maximum > 0) {
    maximum = maximum < 255 ? 255 : maximum;
}
if (minimum > 0){
    ...
    for (index_t y = 0; y < height; y++) {
        for (index_t x = 0; x < width; x++) {
            //auto value = data[(height-1-y)*width+x]; //bottom-up
            auto value = data[y*width+x]; // top-down same as in the data-matrix-format
            if(scale){
                auto color = cmap.bgr(value, minimum, maximum);
                ofile.write((char *) &color, 3);
            } else {
                auto color = cmap.bgr(value);
                ofile.write((char *) &color, 3);
            }
